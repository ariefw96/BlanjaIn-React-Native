...
class EditStock extends React.Component {
    ...
    getUpdateData = () => {
        axios.get(BASE_URL + '/product/getStockData/' + this.props.route.params.itemId, {
            headers: {
                "x-access-token": "Bearer " + this.props.auth.token
            },
        }).then(({ data }) => {
            this.setState({
                selectedProduct: data.data.product_id,
    ...
    addStock = () => {
        ...
        axios.patch(BASE_URL + '/product/updateStock/'+this.props.route.params.itemId, data, config)
            .then(({ data }) => {
                console.log(data)
                alert(data.msg)
